...
if __name__ == "__main__":
    ...
    cam.MV_CC_SetFloatValue("ExposureTime",7000)
    ...
    while not rospy.is_shutdown():
        ...
        if max_size == 0: #压根不存在装甲板
            ...
            if len(best_light) > 0:
                ...
                # for i in range(0,4):
                #     best_light[i][0] = pre_dx + best_light[i][0]
                #     # best_light[i][1] = pre_dy + best_light[i][1]
                #     #y轴不需要卡尔曼，加了卡尔曼反而有问题
                # # 连接四个点形成矩形
                # cv2.line(resized_image, (best_light[0][0], best_light[0][1]), (best_light[1][0], best_light[1][1]), (255, 0, 0), 2)  # 左上到右上
                # cv2.line(resized_image, (best_light[1][0], best_light[1][1]), (best_light[3][0], best_light[3][1]), (255, 0, 0), 2)  # 右上到右下
                # cv2.line(resized_image, (best_light[3][0], best_light[3][1]), (best_light[2][0], best_light[2][1]), (255, 0, 0), 2)  # 右下到左下
                # cv2.line(resized_image, (best_light[2][0], best_light[2][1]), (best_light[0][0], best_light[0][1]), (255, 0, 0), 2)  # 左下到左上
                # cv2.line(resized_image, (best_light[0][0], best_light[0][1]), (best_light[3][0], best_light[3][1]), (255, 0, 0), 2)  # 左上到右上
                # cv2.line(resized_image, (best_light[2][0], best_light[2][1]), (best_light[1][0], best_light[1][1]), (255, 0, 0), 2)  # 右上到右下
                ...
                angle1 = angle1 - 5  #PITCH
                angle2 = angle2 + 3.5  #YAW
                length = max(calculate_distance((best_light[2][0],best_light[2][1]),(best_light[0][0],best_light[0][1]))
                            ,calculate_distance((best_light[1][0],best_light[1][1]),(best_light[3][0],best_light[3][1]))) * 1.0

                my_text1 = "pitch:"+str(angle1)
                my_text2 = "yaw:"+str(angle2)
                my_text3 = "distance"+str(length)
                my_text4 = "IOU"+str(chonghe_rate)
                my_text5 = "last"+str(last_target_x1+last_target_x2-target_x1-target_x2)
                if success:
        ...
        # data1 = 0.0
        shoot = 0
        if len(best_light) > 0 and abs(angle1) < 0.7 and abs(angle2) < 0.7:
            shoot = 1

        get_target = 1

        if len(best_light) > 0: #有目标
            # get_target = 1
            lost_target_times = 0
        else:
            lost_target_times = lost_target_times + 1
            
            if lost_target_times > 25:
                get_target = 0
        # get_target_last = get_target


        array = Float32MultiArray(data=[1000 * math.radians(angle1), -1000 * math.radians(angle2),shoot,get_target])
