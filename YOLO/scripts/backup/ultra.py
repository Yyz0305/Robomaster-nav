...
if __name__ == "__main__":
    ...
    # 加载预训练的YOLOv8模型，并将其移动到GPU上（如果可用）
    # model = YOLO("//home//ubuntu//catkin_ws//src//YOLO//scripts//20000best.pt")  # 使用'nano'大小的模型作为示例
    model.to(device)
    ...
    while not rospy.is_shutdown():
        ...
        image_tensor = torch.from_numpy(image_rgb).permute(2, 0, 1).float().unsqueeze(0).to(device)

        # 使用YOLOv8进行推理
        results = model(image_tensor)  # 推理结果
        ...
        cv2.imshow("image", mat=annotated_frame)

        # 这里假设你已经有了计算得到的y和z值
        y = 1.0  # 示例数据，请替换为实际获取的数据
        z = 2.0  # 示例数据，请替换为实际获取的数据
        
        # 构造消息
        array = Float32MultiArray(data=[y, z])
