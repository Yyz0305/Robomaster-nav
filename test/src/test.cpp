...
void cameraCallback(const std_msgs::Float32MultiArray::ConstPtr& msg) {
    ...
    if (msg->data.size() >= 3) { // 确保至少有两个元素
      if(global_flag == 0){
        ...
        shoot = 0x01; 
        if(shootf == 0){//还没跟踪到，拨弹不开
          shoot = 0x01;
        }else if(shootf == 1){//跟踪到，开拨弹
          shoot = 0x02;
        }
        // shoot = 0x00;
        ROS_INFO("Received pitch=%f, yaw=%f,shoot=%f", pitch, yaw, shootf);
        if(get_targetf == 0)//没有目标
        {
          send_date(0, 0, 2, (int)yaw, (int)pitch, 0x01); // 巡检模式
        }else if (get_targetf == 1)//有的兄弟，有的 
        {
          send_date(0, 0, 0, (int)yaw, (int)pitch, 0x02); // 瞄准模式
        }
...
void cmdVelCallback(const geometry_msgs::Twist::ConstPtr& msg)
{
  ...
  int angular_z = -msg->angular.z * 5000;
...
int main(int argc, char **argv) {
    ...
    ros::spin();
    // while(ros::ok()){    
    //     send_date(0,0,2,0,0,0x00); //yaw往左为正数，往右为负数
    //     rate.sleep();
    // }
